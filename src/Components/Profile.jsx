...
export default function Profile() {
  ...
  const toggleMessageVisibility = async () => {
    ...
    setCheckMessages(prevCheckMessages => !prevCheckMessages);
  };
  ...
  return (
    <div>
      ...
      <button onClick={toggleMessageVisibility}>
        {checkMessages ? 'Hide Messages' : 'Show Messages'}
      </button>
      <div>
        {checkMessages && yourMessages.length > 0 ? (
          <ul>
            ...
            {yourPosts.map(post => (
              <li key={post._id}>
                {editingPostId === post._id ? (
                  <>
                    <button onClick={() => handleEditSubmit(post._id, editedPosts)}>Save Changes</button>
